...
public class Users implements IUsers{
    ...
    public User getUser(String username, String password) {
        ...
        String sql = "SELECT x1."+ COL_USER_ID +", ucase(x1."+ COL_FULLNAME +") fullname, x1."+ COL_USERNAME +", x1."+ COL_USERGROUP_ID +", x1."+ COL_PASSWORD +", x2."+ COL_USRFILE_BLOB +" " +
                "FROM "+ TBL_USERS +" x1 " +
                "left outer join "+ TBL_USERS_PHOTOS +" x2 ON x1."+ COL_USER_ID +" = x2."+ COL_USER_ID +" " +
    ...
    public ObservableList<User> getUsers(String param) {
        ...
        String sql = "SELECT x1."+ COL_USER_ID +",ucase(x1."+ COL_FULLNAME +") "+ COL_FULLNAME +",x1."+ COL_USERNAME +
                ",x1."+ COL_USERGROUP_ID +",x1."+ COL_PASSWORD +", x2."+ COL_USRFILE_BLOB +" " +
                "FROM "+ TBL_USERS + " x1 " +
                "left outer join "+ TBL_USERS_PHOTOS +" x2 on x1."+ COL_USER_ID +" = x2."+ COL_USER_ID + " " +
    ...
    public ExecuteStatus handleUser(User user) {
        ...
        try {
            ...
            ps.close();
            sql = "delete from "+ TBL_USERS_PHOTOS +" where "+ COL_USER_ID +" = " + user.getUserID();
            ps = ConnectionMain.getInstance().cn().prepareStatement(sql);
            ps.executeUpdate();

            if(user.getUserPhoto() != null) {
                sql = "insert into "+ TBL_USERS_PHOTOS +" ("+ COL_USER_ID +","+ COL_USRFILE_SIZE +", "+ COL_USRFILE_BLOB +") values (?,?,?)";
                ps = ConnectionMain.getInstance().cn().prepareStatement(sql);
